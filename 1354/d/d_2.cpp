    ...
    void insert(int k, int l, int r, int x)
    {
        ...
        int mid = (l + r) / 2;
        if (x <= mid) //寻找 x 对应的计数器
            insert(k * 2, l, mid, x);
        else
            insert(k * 2 + 1, mid + 1, r, x);
        cnt[k] = cnt[k * 2] + cnt[k * 2 + 1]; //更新区间计数
        return;
    }
    ...
    int del(int k, int l, int r, int rnk)
    {
        ...
        int mid = (l + r) / 2, res;
        if (cnt[k * 2] >= rnk) //用区间计数和排名比较寻找对应计数器
    ...
    void solve()
    {
        ...
        write(cnt[1] ? del(1, 1, n, 1) : 0, endl); // cnt[1] 是总的数量
    }